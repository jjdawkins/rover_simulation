...
class ignition::gazebo::systems::RoverPluginPrivate
{
...
RoverPlugin::RoverPlugin()
  : dataPtr(std::make_unique<RoverPluginPrivate>())
{
}

//////////////////////////////////////////////////
void RoverPlugin::Configure(const Entity &_entity,
    ...
    EventManager &/*_eventMgr*/)
{
  ...
  {
    ignerr << "RoverPlugin should be attached to a model entity. "
  ...
  if (topic.empty())
  {
    ignerr << "RoverPlugin plugin received invalid model name "
           << "Failed to initialize." << std::endl;
    return;
  }

  this->dataPtr->node.Subscribe(topic, &RoverPluginPrivate::OnCmdVel,
      this->dataPtr.get());
  ...
  if (topic.empty())
  {
    ignerr << "RoverPlugin received invalid model name "
           << "Failed to initialize." << std::endl;
    return;
  }
  ...
  ignmsg << "RoverPlugin subscribing to twist messages on [" <<
      topic << "]" << std::endl;
}
...
{
  IGN_PROFILE("RoverPlugin::PostUpdate");
  // Nothing left to do if paused.
  if (_info.paused)
    return;
...
void AckermannSteeringPrivate::UpdateOdometry(
    ...
    const ignition::gazebo::EntityComponentManager &_ecm)
{
  IGN_PROFILE("RoverPlugin::UpdateOdometry");
...
//////////////////////////////////////////////////
void RoverPluginPrivate::UpdateVelocity(
    const ignition::gazebo::UpdateInfo &_info,
    const ignition::gazebo::EntityComponentManager &_ecm)
{
  IGN_PROFILE("RoverPlugin::UpdateVelocity");
...
//////////////////////////////////////////////////
void RoverPluginPrivate::OnCmdVel(const msgs::Twist &_msg)
{
  std::lock_guard<std::mutex> lock(this->mutex);
  this->targetVel = _msg;
}

IGNITION_ADD_PLUGIN(RoverPlugin,
                    ignition::gazebo::System,
                    RoverPlugin::ISystemConfigure,
                    RoverPlugin::ISystemPreUpdate,
                    RoverPlugin::ISystemPostUpdate)

IGNITION_ADD_PLUGIN_ALIAS(RoverPlugin,
                          "ignition::gazebo::systems::RoverPlugin")
